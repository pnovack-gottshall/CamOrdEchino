## Version of ReadMorphNexus from Graeme Lloyd's GitHub, which is not apparently
## playing well when downloaded from GitHub (perhaps due to R 4.0 encoding of
## UTF?) This version always (instead of sometimes) works--notice change in line
## 254). This version also has wonderfully helpful comments. Downloaded 7/2/2020.
...
## WHERE PROCESSING BEGINS ##################################################### 
  ...
  for(i in 1:length(MatrixBlockList)) {
    ...
    TitleLine <- intersect(which(unlist(lapply(lapply(strsplit(CurrentBlock, split = ""), '[', 1:5), paste, collapse = "")) == "TITLE"), 
                           1:which(unlist(lapply(lapply(strsplit(CurrentBlock, split = ""), '[', 1:5), paste, collapse = "")) == "MATRI")[1])
