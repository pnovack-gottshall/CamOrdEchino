## Modified geoscalePlot() function from 'geoscale' package to allow use of ICS
## 2020 timescale. Also allows user to specify plotting figure for multipanel
## layouts and ability to only plot the timescale or plot in user-specified
## figure with user-specified margins using new args 'fig.coords', 'fig.mar',
## 'only.timescale', and 'only.plot'. NOTE THESE MODIFICATIONS ONLY ARE CODED TO
## WORK WITH 'direction' = HORIZONTAL. NOTE THAT WILL CREATE A NEW PLOTTING
## DEVICE WHEN A TIMESCALE IS ADDED BUT NOT WHEN ADDING ONLY THE PLOT. new arg
## 'override.new.fig' allows user to add to existing plotting device. Several
## other changes to accommodate my specific plotting needs. Code by Mark Bell
## and taken from https://github.com/cran/geoscale/blob/master/R/geoscalePlot.R.
## Thanks, Mark!

geoscalePlot3 <- function (ages, data, units = c("Age", "Epoch", "Period"),
                           tick.scale = "myr", boxes = "Age", abbrev, cex.age = 0.3,
                           cex.ts = 0.4, cex.pt = 1, age.lim = NULL, data.lim = NULL, 
                           user.scale, ts.col = TRUE, ts.width = 0.3, label, 
                           timescale = NULL, no.axis = FALSE, direction = "horizontal", 
                           erotate, arotate, urotate, 
                           fig.coords = NULL,
                           fig.mar = NULL, 
                           only.timescale = FALSE, 
                           only.plot = FALSE, 
                           override.new.fig = FALSE,
                           srt = 90,
                           ...)
{
  
  # timescales <- NULL                   # *** MODIFIED ***
  # data(timescales,envir=environment()) # *** MODIFIED ***
  timescale <- timescale                 # *** MODIFIED ***
  
  if (only.timescale & only.plot)        # *** MODIFIED ***
    stop("you can not simultaneusly plot only the timescale and only the data.\n")
  
  if (only.plot) {                       # *** MODIFIED ***
    ts.width <- 0                        # *** MODIFIED ***
    warning("'ts.width' set to 0 because not plotting time scale.\n")
  }                                      # *** MODIFIED ***
  ...
  if(direction == "horizontal"){  
    ...
    # Plot timescale first
    if(!only.plot) {
      if(is.null(fig.coords))                                         # *** MODIFIED ***
        ts.fig.coords <- c(0, 1, 0, (ts.width)) else {                # *** MODIFIED ***
          ts.fig.coords <- fig.coords                                 # *** MODIFIED ***
          ts.fig.coords[4] <- ts.fig.coords[3] + (ts.width * ts.fig.coords[4]) # *** MODIFIED ***
        }                                                             # *** MODIFIED ***
      par(fig = ts.fig.coords, new = override.new.fig)                # *** MODIFIED ***
      if (is.null(fig.mar))                                           # *** MODIFIED ***
        ts.fig.mar <- c(1, 4, 0, 2) else ts.fig.mar <- fig.mar        # *** MODIFIED ***
      par(mar = ts.fig.mar)                                           # *** MODIFIED ***
    ...
    if(!only.timescale) {                                      # *** MODIFIED ***

      if(is.null(fig.coords))                                  # *** MODIFIED ***
        pl.fig.coords <- c(0, 1, (ts.width), 1) else {         # *** MODIFIED ***
          pl.fig.coords <- fig.coords                          # *** MODIFIED ***
          pl.fig.coords[3] <- fig.coords[3] + (ts.width * fig.coords[4])                 # *** MODIFIED ***
        }                                                      # *** MODIFIED ***
      par(fig = pl.fig.coords, new = TRUE)              # *** MODIFIED ***
      if (is.null(fig.mar))                                    # *** MODIFIED ***
        pl.fig.mar <- c(0, 4, 1, 2) else pl.fig.mar <- fig.mar # *** MODIFIED ***
      par(mar = pl.fig.mar)                                    # *** MODIFIED ***
