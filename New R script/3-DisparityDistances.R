...
# Two-column MATRIX, where first column is named "dependent_character" (e.g.,
# the secondary, tertiary, etc. character) and second column is named
# "independent_character" (e.g., the primary character it depends on). Each row
...
# Claddis::calculate_morphological_distances that silences line 396 to avoid triggering
...
(t.start0 <- Sys.time())
distances.MORD <- calculate_morphological_distances(cladistic_matrix = anc,
...
(t.start0 <- Sys.time())
distances.RED <- calculate_morphological_distances(cladistic_matrix = anc, 
                                                   distance_metric = "red")
...
(t.start0 <- Sys.time())
distances.GC <- calculate_morphological_distances(cladistic_matrix = anc, 
                                                  distance_metric = "gc",
                                                  distance_transformation = "arcsine_sqrt")
(Sys.time() - t.start0) # ~ 1 min for life habit, ~ 4 min for morph
beep(3)
...
(t.start0 <- Sys.time())
distances.GED <- calculate_morphological_distances(cladistic_matrix = anc, 
                                                   distance_metric = "ged", 
                                                   ged_type = "wills")
...
(t.start0 <- Sys.time())
distances.GED0 <- calculate_morphological_distances2(cladistic_matrix = anc, 
                                                     distance_metric = "ged", 
                                                     ged_type = "wills", 
                                                     alpha = 0, 
...
# Note that this is run with the "silenced" character dependencies warning.
(t.start0 <- Sys.time())
distances.GED.5 <- calculate_morphological_distances2(cladistic_matrix = anc, 
                                                      distance_metric = "ged", 
                                                      ged_type = "wills", 
                                                      alpha = 0.5, 
                                                      inapplicable_behaviour = "hsj",
...
# Note that this is run with the "silenced" character dependencies warning.
(t.start0 <- Sys.time())
distances.GED1 <- calculate_morphological_distances2(cladistic_matrix = anc, 
                                                     distance_metric = "ged", 
                                                     ged_type = "wills",
                                                     alpha = 1, 
                                                     inapplicable_behaviour = "hsj",
...
for (r in 1:6) {
  for (c in (r+1):7) {
    d1 <- get(matrix.names[r])$distance_matrix
    d2 <- get(matrix.names[c])$distance_matrix
    if (remove.NAs) {
      diag(d1) <- diag(d2) <- NA
